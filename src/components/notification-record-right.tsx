...
export const NotificationRecordRightSide: React.FC<
  NotificationRecordRightSideProps
> = ({ selectedNotificationRecord }) => {
  const [recordDetails, setRecordDetails] =
    useState<NotificationRecordDetails | null>(null);
  ...
  const parseNotificationRecordDetails = (
    output: string,
    targetKey: string
  ): NotificationRecordDetails | null => {
    ...
    for (
      let i = recordStartIndex;
      i < Math.min(recordStartIndex + 100, lines.length);
      i++
    ) {
      const line = lines[i];
      ...
      if (recordMatch) {
        ...
        details.importance = Number.parseInt(recordMatch[5]);
      ...
      if (shortcutMatch) {
        details.shortcut = shortcutMatch[1];
      }

      const contentViewMatch = line.match(/contentView=([^ ]+)/);
      if (contentViewMatch) {
        details.contentView = contentViewMatch[1];
      }

      const vibrateMatch = line.match(/vibrate=([^ ]+)/);
      if (vibrateMatch) {
        details.vibrate = vibrateMatch[1];
      }

      const soundMatch = line.match(/sound=([^ ]+)/);
      if (soundMatch) {
        details.sound = soundMatch[1];
      }

      const defaultsMatch = line.match(/defaults=(\d+)/);
      if (defaultsMatch) {
        details.defaults = Number.parseInt(defaultsMatch[1]);
      }

      const flagsMatch = line.match(/flags=([^ ]+)/);
      if (flagsMatch) {
        details.flagsRaw = flagsMatch[1];
      }
      ...
      if (visMatch) {
        details.visibilityRaw = visMatch[1];
      }

      const groupKeyMatch = line.match(/groupKey=([^ ]+)/);
      if (groupKeyMatch) {
        details.groupKey = groupKeyMatch[1];
      }

      const sortKeyMatch = line.match(/sortKey=([^ ]+)/);
      if (sortKeyMatch) {
        details.sortKey = sortKeyMatch[1];
      }

      const actionsMatch = line.match(/actions=(\d+)/);
      if (actionsMatch) {
        details.actions = Number.parseInt(actionsMatch[1]);
      }
      ...
      if (whenMatch) {
        details.when = Number.parseInt(whenMatch[1]);
      }

      const createTimeMatch = line.match(/creationTime=(\d+)/);
      if (createTimeMatch) {
        details.createTime = Number.parseInt(createTimeMatch[1]);
      }

      const updateTimeMatch = line.match(/updateTime=(\d+)/);
      if (updateTimeMatch) {
        details.updateTime = Number.parseInt(updateTimeMatch[1]);
      }

      // Parse uid, opPkg, postTime from subsequent lines
      const uidMatch = line.match(/uid=(\d+)/);
      if (uidMatch) {
        details.uid = Number.parseInt(uidMatch[1]);
      }

      const opPkgMatch = line.match(/opPkg=(.+)/);
      if (opPkgMatch) {
        details.opPkg = opPkgMatch[1];
      }

      const postTimeMatch = line.match(/postTime=(\d+)/);
      if (postTimeMatch) {
        details.postTime = Number.parseInt(postTimeMatch[1]);
      }
      ...
      if (subTextMatch) {
        details.subText = subTextMatch[1];
      }

      const largeIconMatch = line.match(/largeIcon=([^=\s]+)/);
      if (largeIconMatch) {
        details.largeIcon = largeIconMatch[1];
      }

      const smallIconMatch = line.match(/smallIcon=([^=\s]+)/);
      if (smallIconMatch) {
        details.smallIcon = smallIconMatch[1];
      }

      const ledARGBMatch = line.match(/ledARGB=(0x[0-9a-fA-F]+)/);
      if (ledARGBMatch) {
        details.ledARGB = Number.parseInt(ledARGBMatch[1]);
      }

      const ledOnMSMatch = line.match(/ledOnMS=(\d+)/);
      if (ledOnMSMatch) {
        details.ledOnMS = Number.parseInt(ledOnMSMatch[1]);
      }

      const ledOffMSMatch = line.match(/ledOffMS=(\d+)/);
      if (ledOffMSMatch) {
        details.ledOffMS = Number.parseInt(ledOffMSMatch[1]);
      }

      const iconResMatch = line.match(
        /icon=Icon\(typ=RESOURCE pkg=([^ ]+) id=([^)]+)\)/
      );
      if (iconResMatch) {
        details.iconRes = `${iconResMatch[1]}:${iconResMatch[2]}`;
      }

      // Stop parsing when we hit the next record
      if (
        i > recordStartIndex &&
        line.includes("NotificationRecord(") &&
        !line.includes(targetKey)
      ) {
        break;
      }
    }
  ...
  useEffect(() => {
    ...
    const fetchRecordDetails = async () => {
      ...
      try {
        ...
        const details = parseNotificationRecordDetails(
          output,
          selectedNotificationRecord
        );
        ...
        if (details) {
          ...
          for (let i = 0; i < lines.length; i++) {
            ...
            if (
              recordStartIndex !== -1 &&
              i > recordStartIndex &&
              lines[i].includes("NotificationRecord(")
            ) {
              recordEndIndex = i;
              break;
            }
          ...
          if (recordStartIndex !== -1) {
            const recordLines = lines.slice(
              recordStartIndex,
              recordEndIndex !== -1 ? recordEndIndex : recordStartIndex + 80
            );
  ...
  return (
    <div className="mr-2 mb-2 ml-0 min-h-full min-w-0 flex-1">
      {selectedNotificationRecord ? (
                ...
                <div className="flex-1 overflow-auto">
                  <TabsContent className="mt-0 p-4" value="details">
                    <div className="space-y-6">
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-4 text-xs">
                          ...
                          <div>
                            ...
                            <p className="font-mono text-muted-foreground">
                              {recordDetails.tag || "N/A"}
                            </p>
                          </div>
                          <div>
                            <span className="font-medium">Importance:</span>
                            <p className="text-muted-foreground">
                              {formatImportance(recordDetails.importance)}
                            </p>
                          </div>
                          <div>
                            <span className="font-medium">Visibility:</span>
                            <p className="text-muted-foreground">
                              {formatVisibility(recordDetails.visibilityRaw)}
                            </p>
                          </div>
                          <div>
                      ...
                      {/* Content */}
                      <div>
                        <h3 className="mb-3 font-medium text-sm">Content</h3>
                      ...
                      {/* Behavior */}
                      <div>
                        <h3 className="mb-3 font-medium text-sm">Behavior</h3>
                        <div className="space-y-2 text-xs">
                          <div>
                            <span className="font-medium">Flags:</span>
                            <p className="text-muted-foreground">
                              {formatFlags(recordDetails.flagsRaw)}
                            </p>
                          </div>
                          {recordDetails.sound && (
                            <div>
                            ...
                            <div>
                              ...
                              <div className="flex items-center space-x-2">
                                <div
                                  ...
                                  style={{
                                    backgroundColor: recordDetails.color,
                                  }}
                                />
                      ...
                      {/* Timing */}
                      <div>
                        <h3 className="mb-3 font-medium text-sm">Timing</h3>
                        <div className="space-y-2 text-xs">
                          <div>
                            <span className="font-medium">Posted:</span>
                            <p className="text-muted-foreground">
                              {formatTimestamp(recordDetails.postTime)}
                            </p>
                          </div>
                          {recordDetails.when && (
                            <div>
                              ...
                              <p className="text-muted-foreground">
                                {formatTimestamp(recordDetails.when)}
                              </p>
                            </div>
                          )}
                          {recordDetails.createTime && (
                            <div>
                              ...
                              <p className="text-muted-foreground">
                                {formatTimestamp(recordDetails.createTime)}
                              </p>
                            </div>
                          )}
                          {recordDetails.updateTime && (
                            <div>
                              ...
                              <p className="text-muted-foreground">
                                {formatTimestamp(recordDetails.updateTime)}
                              </p>
                            </div>
                          )}
                        </div>
        ...
        <div className="flex h-full items-center justify-center">
          <p className="text-muted-foreground text-sm">
            Select a notification record to view details
          </p>
        </div>
      )}
    </div>
